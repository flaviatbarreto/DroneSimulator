	...
	while ((line = lines[index++]) != null) {
		...
		switch(command) {
			...
			case 'usemtl'	: // Read Material name
								...
								currentObject.material = currentMaterialName;
								break; // Go to the next line
			case 'f'		: // Read face
								var face = this.parseFace(sp, this.vertices, reverse);
								currentObject.addFace(face);
								break; // Go to the next line
			case 'group'	:   // Read group material
								currentObjName = sp.getWord();
								currentObject = this.findObject(currentObjName);
...
function onReadMTLFile(fileString, mtl) {
  ...
  while ((line = lines[index++]) != null) {
	...
	switch(command){
		case '#'		:	break;    // Skip comments
		case 'newmtl'	: 	name = mtl.parseNewmtl(sp);    // Get name
							break; // Go to the next line
		case 'Kd'		:   if(name == "") {
								break;
...
OBJDoc.prototype.findColor = function(name) {
	...
	for(var i = 0; i < this.mtls.length; i++) {
		for(var j = 0; j < this.mtls[i].materials.length; j++) {
			var strMatName = this.mtls[i].materials[j].name.trim();
			if(strMatName == strName) {
...
var Min = new Object();
	Min.x = this.vertices[0].x;
	Min.y = this.vertices[0].y;
	Min.z = this.vertices[0].z;
var Max = new Object();
	Max.x = Min.x;
	Max.y = Min.y;
	Max.z = Min.z;
var BBox = new Object();
	...
	for(var i = 0; i < this.objects.length; i++){
		...
		var color = this.findColor(this.objects[i].name);
  ...
  return new DrawingInfo(vertices, normals, colors, indices, BBox);
}
...
OBJObject.prototype.addMaterial = function(materialName) {
...
// DrawInfo Object
//------------------------------------------------------------------------------
var DrawingInfo = function(vertices, normals, colors, indices, BBox) {
